...
class Data_process(object):
    ...
    def create_playlist2(self):
        ...
        with self.connection.cursor() as cursor:
            ...
            for course in courses:
                sql = "select * from tbl_video as a where a.video_id is not null and a.course_id='"+str(course['id'])+"' order by a.position asc limit 49;"
                cursor.execute(sql)
                videos = cursor.fetchall()
                video_ids = ''
                for video in videos:
    ...
    def update_playlist_description(self):
        ...
        with self.connection.cursor() as cursor:
            # Read a single record title like '" + ctitle + "%' and
            sql = "select a.* from tbl_course as a where a.list_id is not null and a.is_update_playlist_des=0;"
            cursor.execute(sql)
